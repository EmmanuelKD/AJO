...
export default function TabsLayout({
  ...
  return (
        ...
        >
          <Tab.List
            className={
              "h-14  justify-center items-center gap-36 flex border-b-2 border-zinc-200 w-full"
            }
          >
            {keys.map((key) => (
              <Tab
              ...
              >
                ...
                <span className=" text-zinc-900 text-base font-medium font-['League Spartan'] leading-none">
                  {key}
                </span>
              </Tab>
            ))}
          </Tab.List>
          <Tab.Panels as="div" className="w-full h-full">
            {keys.map((key) => (
              <Tab.Panel as="div" className="w-full h-full" key={key}>{pannels.get(key)?.child}</Tab.Panel>
            ))}
          </Tab.Panels>
        </Tab.Group>
   );
